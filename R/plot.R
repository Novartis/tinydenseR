...
#' Plot nmfDE Scores (Diagnostic and Component Views)
#'
#' Visualize nmfDE results: either a diagnostic overview of all components
#' or a detailed component-level view showing UMAP embedding and Y-vs-score
#' scatter.
#'
#' When component is NULL, displays a diagnostic overview of all components.
#'
...
#' @param .embed Character: either \code{"umap"} or \code{"pca"}. Default "umap".
#' @param .point.size Numeric: point size for scatter plots. Default 1.
#' @param .label.size Numeric: label size for diagnostic scatter plots. Default 3. Applies only if .nmfDE.dim is NULL.
#' @param .panel.size Numeric: panel size in inches. Default 2.
#' @param .seed Integer: random seed for point ordering. Default 123.
#'
#' @return A ggplot2 object (if \code{.nmfDE.dim = NULL}) or a patchwork composition
#'   (if \code{.nmfDE.dim} is integer).
#'
#' @details
#' \strong{Component view} (\code{.nmfDE.dim = integer}):
#' \itemize{
#'   \item Left panel: UMAP embedding colored by nmfDE signed scores (diverging scale)
#'   \item Right panel: Scatter of Y vs signed scores (essential for interpretation)
#' }
#'
#' \strong{Diagnostic view} (\code{.nmfDE.dim = NULL}):
#' \itemize{
#'   \item X-axis: Smoothness (Sk); higher = large-scale graph-smooth structure
#'   \item Y-axis: Y-alignment (Ak); higher = stronger density coupling
#'   \item Color: NMF scaling diagonal d_k (component strength)
#'   \item Labels: Component names (nmfDE1, nmfDE2, ...)
#' }
#'
#' \strong{Interpreting the diagnostic plot:}
#' \itemize{
...
#'
#' # Diagnostic overview
#' plotNmfDE(lm.obj, .coef.col = "Infection", .nmfDE.dim = NULL)
#'
#' # Visualize first component
...
plotNmfDE.TDRObj <-
  function(
    ...
    if (is.null(x = .nmfDE.dim)) {
      
      diag.df <-
        data.frame(
          component = gsub(pattern = "nmfDE",
                           replacement = "",
                           x = names(x = nmfDE.res$smoothness)),
          Sk = nmfDE.res$smoothness,
          Ak = nmfDE.res$Y.alignment,
          dk = nmfDE.res$d
        )
      ...
      p <-
        ...
        ggplot2::scale_color_gradient(low = unname(obj = Color.Palette[1,1]),
                                      ...
                                      name = expression(d[k])) +
        ggplot2::labs(
          title = paste0("nmfDE diagnostics: ", .coef.col),
          subtitle = sprintf("Rk (global) = %.1f%%", nmfDE.res$reconstruction * 100),
          x = "Smoothness (Sk)",
          y = "Y-alignment (Ak)"
        ) +
        ggplot2::theme_bw() +
    ...
    # -------------------------------------------------------------------------
    # Component view: UMAP + Y-vs-signed.score scatter
    # -------------------------------------------------------------------------
    ...
    # Panel 2: Y vs signed score scatter
    p.scatter <-
      ggplot2::ggplot(data = scatter.df,
                      mapping = ggplot2::aes(x = Y,
                                             y = score,
      ...
      ggplot2::labs(
        title = paste0(.coef.col, " vs ", comp.name),
        subtitle = sprintf("Ak = %.2f, Sk = %.2f, d = %.1f",
                           nmfDE.res$Y.alignment[.nmfDE.dim],
                           nmfDE.res$smoothness[.nmfDE.dim],
                           nmfDE.res$d[.nmfDE.dim]),
        x = paste0("Density contrast", "\n(centered ", .coef.col, ")"),
        y = comp.name,
        color = paste0(.coef.col,
                       " (raw)")
...
#' Plot nmfDE Expression Heatmap
#'
#' Creates an efficient expression heatmap with landmarks as columns and top-loaded
#' features as rows. Landmarks are ordered by nmfDE signed scores (or custom ordering),
#' and features are ranked by absolute loading. For RNA data, expression is properly
#' normalized, log-transformed, and centered.
#'
#' @param x A tinydenseR object after \code{get.nmfDE()}.
...
#'     \item{\code{"dens.contrast"}}{(default) Order by centered density contrast
#'       from \code{.coef.col}.}
#'     \item{\code{"nmfDE.dim"}}{Order by the selected \code{.nmfDE.dim} signed score(s).}
#'     \item{A numeric matrix with column names}{Landmarks are sorted by columns
#'       sequentially (first column primary, etc.) and each column is displayed as
#'       an annotation strip with the column name as legend title.}
#'   }
...
#' @param .order.decreasing Logical: sort landmarks in decreasing order? Default FALSE.
#' @param .viridis.options.annot Character vector: viridis color options for annotation
#'   strips (density contrast + nmfDE dimensions + .order.by columns). Cycled if fewer
#'   options than strips. Default \code{c("cividis", "rocket", "inferno", "mako", "magma")}.
#' @param .annot.panel.width Numeric: width of annotation strips in inches. Default 4.
#' @param .annot.panel.height Numeric: height of each annotation strip in inches. Default 0.15.
#' @param .panel.width Numeric: width of expression heatmap panel in inches. Default 4.
...
#' @details
#' \strong{Feature selection and ordering}:
#' Features are \emph{selected} by absolute loading value for the first element of \code{.nmfDE.dim}
#' (for RNA, the top \code{.n.features}; for cytometry, all markers). However, the selected features 
#' are then \emph{ordered} by signed loading (highest positive at top, most negative at bottom), 
#' making it easy to see which genes are up vs down-regulated along the nmfDE component.
#'
...
#' \strong{Landmark ordering}:
#' When \code{.order.by} is multi-dimensional (matrix with >1 column), landmarks are
#' sorted by the first column, then by the second within ties, etc.
#'
#' \strong{Annotation strips}:
#' The following are shown as annotation strips at the top:
#' \enumerate{
#'   \item Density contrast (centered Y from \code{get.lm})
#'   \item nmfDE signed scores for all dimensions in \code{.nmfDE.dim}
#'   \item If \code{.order.by} is provided: each column of \code{.order.by}, with
#'     column names used as legend titles
#' }
#' This ensures the relationship between expression patterns and the nmfDE structure
#' is always visible, plus any custom ordering variables.
#'
#' \strong{Visualization}:
#' Uses \code{ggplot2::geom_raster()} for efficient rendering of large heatmaps.
...
#' sequential viridis scales.
#'
#' @seealso \code{\link{get.nmfDE}} for computing nmfDE, \code{\link{plotNmfDE}}
#'   for score visualization
#'
#' @examples
#' \dontrun{
...
#' lm.obj <- get.nmfDE(lm.obj, .coef.col = "Infection")
#'
#' # Basic heatmap using nmfDE1
#' plotNmfDEHeatmap(lm.obj, .coef.col = "Infection", .nmfDE.dim = 1)
#'
#' # Order by two dimensions (e.g., to see 2D structure)
#' plotNmfDEHeatmap(lm.obj, .coef.col = "Infection", .nmfDE.dim = c(1, 2))
#'
#' # Use mass loadings instead of DE loadings
#' plotNmfDEHeatmap(lm.obj, .coef.col = "Infection", .nmfDE.dim = 1,
#'                  .loadings.type = "mass")
#'
#' # Order by nmfDE dimension scores
#' plotNmfDEHeatmap(lm.obj, .coef.col = "Infection", .nmfDE.dim = 1,
#'                  .order.by = "nmfDE.dim")
#' }
#'
