...
The `dgCMatrix`, `DelayedMatrix`, `IterableMatrix`, `flowSet`, and `cytoset`
backends produce a bare `TDRObj` directly — they do not wrap the result back
into a container object. After calling `RunTDR()`, you work with the `TDRObj`
directly:
...
The `flowSet` and `cytoset` backends (from `flowCore` and `flowWorkspace`,
respectively) are designed for flow, mass, and spectral cytometry data and
require `.assay.type = "cyto"` along with a `.markers` argument specifying
the channels to use. Both formats use the same internal backend
(`"cytoset"`), so downstream behaviour is identical.
