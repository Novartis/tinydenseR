...
### Density Analysis: Identifying Affected Subpopulations

Before performing differential expression analysis, we first run `get.lm()` to identify which cell states might be affected by the experimental design. This guides us to the appropriate subpopulations for subsequent DE testing.
...
# Access results via lm.cells.DE.0.5$map$lm[["default"]]
```

#### Visualize Density Changes

Let's visualize density changes to identify regions that might show differential expression:
...
Now that we've identified cluster 3 as the target population, we can perform differential expression analysis within this cluster using `get.pbDE()`:

```{r dea-cluster3}
# Perform differential expression analysis for cluster 3 (target cells)
lm.cells.DE.0.5 <- tinydenseR::get.pbDE(
  .tdr.obj = lm.cells.DE.0.5,
  .design = .design.0.5,
  .id = "cluster.3",
  .population.name = "cluster3"
)
```
...
# View coefficients
lm.cells.DE.0.5$pbDE$default$cluster3$coefficients
```

```{r dea-results-pvalues}
# View adjusted p-values
lm.cells.DE.0.5$pbDE$default$cluster3$adj.p
```
...
As we saw in section **Visualize Density Changes**, the affected population separate well along PC1, which is primarily driven by Markers 1, 4 and 5.
...
lm.cells.DE.0.5 <- tinydenseR::get.pbDE(
    .tdr.obj = lm.cells.DE.0.5,
    .design = .design.0.5,
    .id.idx = which(x = (lm.cells.DE.0.5$landmarks[,"Marker1"] > log(x = 10)) &
                        (lm.cells.DE.0.5$landmarks[,"Marker5"] > log(x = 100))),
    .population.name = "marker_defined"
)
```

```{r dea-cluster-independent-res}
# View coefficients for all markers
lm.cells.DE.0.5$pbDE$default$marker_defined$coefficients
# View adjusted p-values for all markers
lm.cells.DE.0.5$pbDE$default$marker_defined$adj.p
...
Above we identified differential cell states within cluster 3. Let's use `get.pbDE(.mode = "marker")` to identify which markers define this cluster compared to all other cells:

```{r marker-identification}
# Identify markers for cluster 3 (results stored in lm.cells.DE.0.5$markerDE)
lm.cells.DE.0.5 <-
  tinydenseR::get.pbDE(
    x = lm.cells.DE.0.5,
    .mode = "marker",
...
lm.cells.DE.0.5$markerDE$default$cluster3_markers$coefficients[, c("(Intercept)", ".id1")]
```

```{r marker-pvalues}
# Adjusted p-values for marker enrichment
lm.cells.DE.0.5$markerDE$default$cluster3_markers$adj.p[, c("(Intercept)", ".id1")]
```
...
A scatter plot provides flexibility to explore different types of visualizations. Here we plot the distributions of **Marker2 and Marker5** and color the landmarks by density log2FC:

```{r scatter}
tinydenseR::scatterPlot(
  .x.feature = lm.cells.DE.0.5$landmarks[,"Marker2"],
  .x.label = "Marker2",
  .y.feature = lm.cells.DE.0.5$landmarks[,"Marker5"],
  .y.label = "Marker5",
  .color.feature = lm.cells.DE.0.5$map$lm[["default"]]$fit$coefficients[, "Activation"],
  .color.label = "density\nlog2(+0.5) FC",
  .plot.title = "Activation vs Baseline",
  .panel.size = c(3,3),
  .point.size = 2
...
```{r cluster-stats}
tinydenseR::plotTradStats(.tdr.obj = lm.cells.DE.0.5, .model.name = "default")
```
...
stat.test.percentages.DE.0.5 <-
  ...
  dplyr::mutate(
    p = lm.cells.DE.0.5$map$lm[["default"]]$trad$clustering$fit$adj.p[name, "Activation"],
...
`.cache.on.disk = TRUE` and is transparent to downstream analysis.
...
`get.pbDE()`, or any DE method.  You can also validate explicitly:

```{r cache-validate, eval=FALSE}
# Quick file-existence check
tdr_cache_validate(lm.cells)

# Full MD5-checksum verification
tdr_cache_validate(lm.cells, .verify.checksum = TRUE)
```

### Cleaning up

Remove cached files when analysis is complete:

```{r cache-cleanup, eval=FALSE}
lm.cells <- tdr_cache_cleanup(lm.cells)
```

After cleanup, re-run `get.map()` if you need the large slots again.
...
5. **Visualize results** and verify no confounding density changes
...
- See the **Differential Density Analysis** vignette for detecting proportion changes
