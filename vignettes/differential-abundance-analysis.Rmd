...
- Models density at each landmark using **samples as biological replicates**
- Detects regions of differential density without arbitrary clustering
...
Our simulation worked: we see approximately 2-fold depletion (log2FC ≈ -1) from baseline density.

## Differential Density Analysis with tinydenseR
...
1. **`setup.tdr.obj()`**: Initialize landmark object with data
...
Before testing for differential density, let's re-examine the data structure by projecting cells from two representative samples using the PCA computed on landmark cells:
...
tinydenseR::plotPCA(
  .tdr.obj = lm.cells.DA.0.5,
  .feature = lm.cells.DA.0.5$metada$Batch[lm.cells.DA.0.5$config$key],
...
# Run differential density testing
# get.lm() returns the updated .tdr.obj with results in $map$lm[[.model.name]]
lm.cells.DA.0.5 <-
  tinydenseR::get.lm(
    .tdr.obj = lm.cells.DA.0.5,
    .design = .design.0.5
...
# Access results via lm.cells.DA.0.5$map$lm[["default"]]
```

### Visualizing Differential Density Results

#### Density fold changes

We can visualize the log2 fold changes in density at each landmark between Depletion and Baseline conditions:
...
We can also visualize regions of statistically significant density change:
...
A beeswarm plot provides a comprehensive view of density changes across clusters:
...
While differential density analysis reveals which populations change in proportion, we can also examine the **marker expression profiles** that define these populations. This helps us understand the biological identity of the affected cell types.

Above we identified differential cell states within cluster 4. Let's use `get.pbDE(.mode = "marker")` to identify which markers define this cluster compared to all other cells:

```{r marker-identification}
# Identify markers for cluster 4 (results stored in lm.cells.DA.0.5$markerDE)
lm.cells.DA.0.5 <-
  tinydenseR::get.pbDE(
    x = lm.cells.DA.0.5,
    .mode = "marker",
    .id = "cluster.4",
    .result.name = "cluster4_markers"
  )
```

We can visualize how markers are expressed in cluster 4 versus the rest of the population:
...
tinydenseR::plotMarkerDE(
    .tdr.obj = lm.cells.DA.0.5,
    .comparison.name = "cluster4_markers",
...
#               .id1 = difference in cluster 4 vs rest
lm.cells.DA.0.5$markerDE$default$cluster4_markers$coefficients[, c("(Intercept)", ".id1")]
```

```{r marker-pvalues}
# Adjusted p-values for marker enrichment
lm.cells.DA.0.5$markerDE$default$cluster4_markers$adj.p[, c("(Intercept)", ".id1")]
```

As expected, **Marker1, Marker4, and Marker5** show strong positive coefficients for `.id1`, indicating they are highly expressed in cluster 4 compared to other populations. This confirms our simulation design where these markers define the target cell population. Markers 2 and 3, on the other hand, show much less strong expression differences as expected.

### Additional Visualization of Marker Expression

A scatter plot provides flexibility to explore different types of visualizations. Here we show density log2FC across landmarks across the distributions of **Marker1 and Marker5**:

```{r scatter}
tinydenseR::scatterPlot(
  .x.feature = lm.cells.DA.0.5$landmarks[,"Marker1"],
  .x.label = "Marker1",
  .y.feature = lm.cells.DA.0.5$landmarks[,"Marker5"],
  .y.label = "Marker5",
  .color.feature = lm.cells.DA.0.5$map$lm[["default"]]$fit$coefficients[, "Depletion"],
  .color.label = "density\nlog2(+0.5) FC",
  .plot.title = "Depletion vs Baseline",
  .panel.size = c(3,3),
  .point.size = 2
...
```{r cluster-stats}
tinydenseR::plotTradStats(.tdr.obj = lm.cells.DA.0.5, .model.name = "default")
```
...
stat.test.percentages.DA.0.5 <-
  ...
  dplyr::mutate(
    p = lm.cells.DA.0.5$map$lm[["default"]]$trad$clustering$fit$adj.p[name, "Depletion"],
...
## Managing On-Disk Caching

When working with large datasets, `get.map()` stores four large per-sample
slots (clustering IDs, celltyping IDs, nearest landmarks, and fuzzy graphs)
on disk rather than in memory.  This is enabled by default via
`.cache.on.disk = TRUE` and usually requires no user action.

### Inspecting the cache

```{r cache-info, eval=FALSE}
# After get.map(), check what is cached and where
...
Cache files are validated automatically each time you call `get.lm()`,
`get.pbDE()`, or any DE method.  You can also validate manually:

```{r cache-validate, eval=FALSE}
# Quick file-existence check
tdr_cache_validate(lm.cells)

# Full MD5-checksum verification (slower but detects silent corruption)
tdr_cache_validate(lm.cells, .verify.checksum = TRUE)
```

### Cleaning up

When you no longer need the cached files (e.g., analysis is finished and
you have saved your plots/tables), remove them to reclaim disk space:

```{r cache-cleanup, eval=FALSE}
lm.cells <- tdr_cache_cleanup(lm.cells)
```

After cleanup, the large slots become `NULL`.  Re-run `get.map()` to
regenerate them if needed.

### Disabling caching

For small datasets that fit comfortably in memory, skip on-disk caching:

```{r cache-disable, eval=FALSE}
lm.cells <- get.map(lm.cells, .cache.on.disk = FALSE)
```
...
1. **Simulate realistic flow cytometry data** with differential density and batch effects
...
4. **Run statistical tests** for differential density with proper sample-level replication
