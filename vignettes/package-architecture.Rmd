...
`get.map()` already minimises peak memory by excluding large per-sample
objects from its internal accumulator when on-disk caching is enabled
(the default). For additional defence-in-depth, add the following to
your `~/.Renviron` or SLURM/PBS job script **before** launching R:
...
to the OS immediately on `free()`, keeping the process RSS close to
actual R memory usage.
